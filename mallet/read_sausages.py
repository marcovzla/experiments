...
def create_corpus(fname, examples):
    ...
    qq = {}
    qq['slot_mean'] = quantize([f['slot_mean'] for feats in features for f in feats])
    qq['slot_stdev'] = quantize([f['slot_stdev'] for feats in features for f in feats])
    qq['sausage_length'] = quantize([f['sausage_length'] for feats in features for f in feats])
    qq['slot_entropy'] = quantize([f['slot_entropy'] for feats in features for f in feats])
    qq['slot_highest'] = quantize([f['slot_highest'] for feats in features for f in feats])
        ...
        for e in examples:
            for feats, label in zip(e.get_features(), e.get_labels()):
                ...
                for k in sorted(feats):
                    line += '%s=%s ' % (k, feats[k] if k not in qq else best_bin(feats[k], qq[k]))
                line += label
                line += '\n'
                f.write(line)
            f.write('\n')
